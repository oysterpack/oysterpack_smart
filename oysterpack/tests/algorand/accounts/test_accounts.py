...
class MyTestCase(AlgorandTestSupport, unittest.TestCase):
    def test_get_asset_holdings(self):
        ...
        txn = assets.create(
            sender=account,
            asset_name='GOLD', unit_name='GOLD', total_base_units=100000000,
            manager=account, reserve=account,
        ...
        self.assertEqual(len(list(filter(lambda asset_holding: asset_holding.asset_id == asset_id, asset_holdings))),
                         1)

        with self.subTest('lookup asset holdings for new generated account should return an empty list'):
    ...
    def test_get_asset_holding(self):
        ...
        with self.subTest('lookup asset holding for asset that the account does not hold'):
            _pk, account = generate_account()
            asset_holding = get_asset_holding(account, asset_id + 1, self.algod_client)
            self.assertIsNone(asset_holding)

        with self.subTest('lookup asset holding for new generated account should return an empty list'):
            _pk, account = generate_account()
            asset_holding = get_asset_holding(account, asset_id, self.algod_client)
            self.assertIsNone(asset_holding)
