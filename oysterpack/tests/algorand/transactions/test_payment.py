...
class PaymentTestCase(AlgorandTestSupport, unittest.TestCase):
    def test_algo_transfer_transaction(self):
        ...
        with self.subTest(
            "sending the same transaction again should fail because of transaction lease"
        ):
        ...
        signed_txn = self.sandbox_default_wallet.sign_transaction(txn)
        txid = self.algod_client.send_transaction(signed_txn)
        tx_info = algosdk.transaction.wait_for_confirmation(
            algod_client=self.algod_client, txid=txid, wait_rounds=4
        )
        self.assertEqual(
            algosdk.encoding.base64.b64decode(tx_info["txn"]["txn"]["note"]).decode(),
            note,
        )
